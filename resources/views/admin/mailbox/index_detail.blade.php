      ...
      <div class="row">
            ...
            <div class="box-body no-padding">
              <ul class="nav nav-pills nav-stacked">
                ...
                <li><a href="{{ url('admin/mailbox/trash') }}"><i class="fa fa-trash-o"></i> 
                  <span class="label label-danger pull-right">{{ $totaltrashsendcount + $totaltrash }}</span>Trash</a></li>
              </ul>
            </div>
            <!-- /.box-body -->
          </div>
        ...
        <div class="col-md-9">
          <div class="box box-primary">
            ...
            <div id="myDIV" style="border: 1px solid gray;border-radius: 10px; display: none;">
              <form name="formAdd" id="formAdd" method="POST" action="{{ url('/admin/mailbox/farward') }}" enctype="multipart/form-data">
          @csrf
        ...
        <!-- /.col -->
            ...
            <div id="myDIV1" style="border: 1px solid gray;border-radius: 10px; display: none;">
                    <form name="formAdd" id="formAdd" method="POST" action="{{ url('/admin/mailbox/reply') }}" enctype="multipart/form-data">
                @csrf
...
<script type="text/javascript">
  ...
  if(check == true){
    ...
    if(url == 'http://management_system.test/admin/mailbox/'+delete_id+'/index_detail'){
      $.ajax({
        url: '/admin/deletemail/'+delete_id,
        type: 'get',
        success: function(response){
          location.replace("http://management_system.test/admin/mailbox")
        }
      });
    }

    if(url == 'http://management_system.test/admin/mailbox/'+delete_id+'/sent_detail'){
      $.ajax({
        url: '/admin/deletsentemail/'+delete_id,
        type: 'get',
        success: function(response){
